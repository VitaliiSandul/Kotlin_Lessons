...
//    println("totalPrice: $totalPrice")
    ...
    val productsOfInts = ints.fold(1){runningProduct, item -> runningProduct * item}
    println("productsOfInts: $productsOfInts")
    ...
    val sumVeg = groceries.filter { it.category == "Vegetable" }.sumByDouble { it.unitPrice * it.quantity }
    println("sumVeg: $sumVeg")

    val namesTotalLess5 = groceries.filter { it.unitPrice * it.quantity < 5.0 }.map { it.name }
    println("namesTotalLess5: $namesTotalLess5")

    groceries.groupBy { it.category }.forEach {
        println("Category: ${it.key} Total cost: ${it.value.sumByDouble { it.unitPrice * it.quantity }}")
    }

    groceries.filterNot { it.unit == "Bottle" }.groupBy { it.unit }.forEach{
        println("Unit: ${it.key} names: ${it.value.map{it.name}}")
    }
    ...
    myMap.values.groupBy { it }.forEach{println(it)}
